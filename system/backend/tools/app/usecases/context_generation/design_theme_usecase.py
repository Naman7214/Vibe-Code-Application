...
from system.backend.tools.app.references.design_theme_reference import (
    DESIGN_REFERENCE_JSON,
)
...
class DesignThemeUsecase:
    ...
    async def execute(
        self, request: DesignThemeGenerationRequest
    ) -> Dict[str, Any]:
        """Generate design theme using Anthropic service and save to JSON file"""

        try:
            loggers["design_theme"].info(
                f"Starting design theme generation for query: {request.user_query}"
            )
            ...
            loggers["design_theme"].info(
                "📝 Prompt formatted with reference JSON and user query"
            )
            ...
            try:
                ...
                loggers["design_theme"].info(
                    "📊 Generated design theme successfully"
                )

                # Check if it's a design theme structure
                if (
                    isinstance(parsed_json, dict)
                    and "designTheme" in parsed_json
                ):
                    theme_name = parsed_json["designTheme"].get(
                        "name", "Unknown"
                    )
            ...
            loggers["design_theme"].info(
                f"💾 Successfully saved generated design theme to {file_path}"
            )
            ...
            loggers["design_theme"].error(
                f"Error generating design theme: {str(e.detail)}"
            )
            await self.error_repo.insert_error(
                Error(
                    tool_name="design_theme_generation",
            ...
            raise HTTPException(
                status_code=500,
                detail="Error generating design theme: " + str(e.detail),
            )
