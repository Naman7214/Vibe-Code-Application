...
from system.backend.tools.app.references.navigation_context_reference import (
...
class NavigationContextUsecase:
    ...
    async def execute(
        self, request: NavigationContextGenerationRequest
    ) -> Dict[str, Any]:
        """Generate navigation context using Anthropic service and save to JSON file"""

        try:
            loggers["navigation_context"].info(
                f"🧭 Starting navigation context generation using screens from directory: {request.screens_dir}"
            )

            try:
                screens_dir_path = os.path.join(
                    project_root, request.screens_dir
                )
                screens_json = load_screens_from_directory(screens_dir_path)
                loggers["navigation_context"].info(
                    f"📁 Successfully loaded {len(screens_json)} screens from directory."
                )
            except FileNotFoundError:
                error_msg = (
                    f"Screens directory not found: {request.screens_dir}"
                )
            ...
            loggers["navigation_context"].info(
                "📝 Prompt formatted with navigation reference JSON"
            )
            ...
            try:
                ...
                loggers["navigation_context"].info(
                    "📊 Generated navigation context successfully"
                )

                if (
                    isinstance(parsed_json, dict)
                    and "navigationContext" in parsed_json
                ):
                    nav_items_count = len(
                        parsed_json["navigationContext"].get(
                            "navigationItems", []
                        )
                    )
                    loggers["navigation_context"].info(
                        f"📊 Generated {nav_items_count} navigation items"
                    )
                ...
                loggers["navigation_context"].warning(
                    f"⚠️  Warning: Generated content may not be valid JSON: {e}"
                )
                loggers["navigation_context"].info(
                    "💾 Saving raw content anyway..."
                )
            ...
            loggers["navigation_context"].info(
                f"💾 Successfully saved generated navigation context to {file_path}"
            )
            ...
            loggers["navigation_context"].error(
                f"Error generating navigation context: {str(e.detail)}"
            )
            ...
            raise HTTPException(
                status_code=500,
                detail="Error generating navigation context: " + str(e.detail),
            )
