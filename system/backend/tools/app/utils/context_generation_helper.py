...
def save_individual_screens(
    screens: List[Dict[str, Any]], dir_path: str, base_file_name: str
) -> List[str]:
    """
    ...
    for index, screen in enumerate(screens):
        try:
            ...
            loggers["screen_generation"].info(
                f"Successfully saved screen '{screen_id}' to {file_path}"
            )

        except Exception as e:
            loggers["screen_generation"].error(
                f"Failed to save screen {index + 1}: {str(e)}"
            )
            continue
...
def load_screens_from_directory(screens_dir: str) -> List[Dict[str, Any]]:
    ...
    if not json_files:
        raise FileNotFoundError(
            f"No JSON files found in directory: {screens_dir}"
        )
        ...
        try:
            ...
            if isinstance(screen_data, dict):
                ...
                loggers["navigation_context"].info(
                    f"📄 Loaded screen from {os.path.basename(json_file)}"
                )
            else:
                loggers["navigation_context"].warning(
                    f"⚠️  Skipping {os.path.basename(json_file)}: not a JSON object"
                )

        except json.JSONDecodeError as e:
            loggers["navigation_context"].error(
                f"❌ Error parsing JSON file {os.path.basename(json_file)}: {str(e)}"
            )
            raise json.JSONDecodeError(
                f"Invalid JSON in file {json_file}: {str(e)}", "", 0
            )
        except Exception as e:
            loggers["navigation_context"].error(
                f"❌ Error reading file {os.path.basename(json_file)}: {str(e)}"
            )
            raise Exception(f"Error reading file {json_file}: {str(e)}")

    if not screens:
        raise ValueError(
            f"No valid screen objects found in directory: {screens_dir}"
        )

    loggers["navigation_context"].info(
        f"📊 Successfully loaded {len(screens)} screen(s) from {len(json_files)} file(s)"
    )
