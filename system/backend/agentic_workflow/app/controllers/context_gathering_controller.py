...
from system.backend.agentic_workflow.app.usecases.context_gathering_usecase.stage_iii_usecase.stage_iii_usecase import StageIIIUsecase
...
class ContextGatheringController:
    ...
    async def execute(self, request: ContextGatheringRequest) -> JSONResponse:
        ...
        if stage_ii_result["success"]:
            stage_iii_result = await self.stage_iii_usecase.execute(request, dict_of_screens)
