...
from system.backend.agentic_workflow.app.prompts.context_gathering_prompts.stage_iii_prompt import SYSTEM_PROMPT_A, SYSTEM_PROMPT_B, USER_PROMPT_A, USER_PROMPT_B
from system.backend.agentic_workflow.app.services.anthropic_services.llm_service import AnthropicService
...
class Helper:
    ...
    async def run_stage_3_pipeline(self, request: ContextGatheringRequest, dict_of_screens: Dict[str, Any]):
        """
        Processes the input for Stage 3 to define screen requirements.
        """
        if request.is_follow_up:
            result = await self._generate_global_and_screen_components_details(request, dict_of_screens)
            ...
            components_task = self._generate_global_and_screen_components_details(request, dict_of_screens)
    ...
    async def _global_theme_generation(self, request: ContextGatheringRequest):
        ...
        user_prompt = USER_PROMPT_A.format(first_stage_output=json.dumps(first_stage_output, indent=2), second_stage_output=json.dumps(second_stage_output, indent=2))
        response = await self.anthropic_service.generate_text(system_prompt=SYSTEM_PROMPT_A, prompt=user_prompt)
        
        parsed_response = parse_model_output(response)
        
        await self._save_output(request.session_id, parsed_response, "stage_iii_a.json")
    ...
    async def _generate_global_and_screen_components_details(self, request: ContextGatheringRequest, dict_of_screens: Dict[str, Any]):
        """
        ...
        await self._save_output(request.session_id, parsed_response, "stage_iii_b.json")
    ...
    async def _save_output(self, session_id: str, output_data: Dict[str, Any], output_file_name: str):
        """
