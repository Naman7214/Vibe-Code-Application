...
class StageVHelper:
    ...
    async def get_validation_commands(self, platform_type: str) -> List[Dict[str, Any]]:
        """
    ...
    async def execute_validation_command(
        ...
        try:
            ...
            has_errors = (
                ...
                len(parsed_errors) > 0 or
                await self._has_critical_errors(combined_output, command_info["command"])
            )
            ...
            return {
                ...
                "error_details": {
                    ...
                    "message": self._extract_error_message(combined_output, parsed_errors),
        ...
        for pattern in error_patterns:
            ...
            for match in matches:
                ...
                parsed_errors.append({
                    ...
                    "file_path": file_path,
                    "column": column,
                    "context": self._get_error_context(lines, line_number) if line_number else None
                })
    ...
    def _get_error_context(self, lines: List[str], line_number: int) -> Optional[Dict[str, Any]]:
    ...
    def _extract_error_message(self, output: str, parsed_errors: List[Dict[str, Any]]) -> str:
    ...
    async def create_validation_summary(
        ...
        successful_commands = sum(1 for r in validation_results if r["status"] == "success")
        failed_commands = sum(1 for r in validation_results if r["status"] == "error")
        timeout_commands = sum(1 for r in validation_results if r["status"] == "timeout")
        
        total_errors = sum(len(r["error_details"]["parsed_errors"]) for r in validation_results)
        ...
        for result in validation_results:
            summary["command_details"].append({
                ...
                "error_count": len(result["error_details"]["parsed_errors"]),
    ...
    async def update_scratchpads(
        self, session_id: str, validation_results: List[Dict[str, Any]], summary: Dict[str, Any]
    ):
        """
...
"""
                ...
                for i, error in enumerate(result['error_details']['parsed_errors']):
                    validation_report += f"""
<ERROR_{i+1}>
...
"""
        ...
        validation_results_path = os.path.join(scratchpads_dir, "validation_results.json")
        ...
        self.logger.info(f"Updated validation results at {validation_results_path}")
