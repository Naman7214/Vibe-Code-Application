        ...
        for result in validation_results:
            if result["has_errors"]:
                ...
                if (
                    ...
                    if (
                        "Target dart2js failed" in raw_output
                        or "Error: Compilation failed" in raw_output
                    ):
                        error_message = (
                            self.helper._extract_flutter_compilation_errors(
                                raw_output
                            )
                        )
    ...
    async def execute(self, request: CodeGenerationRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            if has_errors:
                ...
                simple_error = self._create_simple_error_message(
                    validation_results
                )
                return {
                    "success": False,
