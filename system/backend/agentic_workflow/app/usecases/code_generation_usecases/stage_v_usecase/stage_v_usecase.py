...
class StageVUsecase:
    ...
    async def execute(self, request: CodeGenerationRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_commands = await self.helper.get_validation_commands(platform_type)
            ...
            await self.error_repo.insert_error(
                Error(
                    ...
                    error_message="Error in Stage V code validation usecase: " + str(e.detail),
                    stack_trace=str(e),
                )
            )
            return {
                "success": False,
                "message": "Error in Stage V code validation usecase: " + str(e.detail),
                "error": e.detail,
            }
        except Exception as e:
            await self.error_repo.insert_error(
                Error(
                    phase="code_generation_stage_v",
                    error_message="Unexpected error in Stage V code validation usecase: " + str(e),
                    stack_trace=str(e),
                )
            )
            return {
                "success": False,
                "message": "Unexpected error in Stage V code validation usecase: " + str(e),
                "error": str(e),
            }
