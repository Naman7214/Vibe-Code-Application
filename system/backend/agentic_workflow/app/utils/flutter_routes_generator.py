...
class FlutterRoutesGenerator:
    ...
    def _is_initial_screen(self, screen_name: str) -> bool:
        ...
        """
        initial_indicators = [
            'splash_screen', 'splash', 'home', 'home_dashboard', 
            'main', 'dashboard', 'index', 'landing'
        ]
    ...
    def generate_app_routes_dart(self) -> str:
        ...
        for screen in screens:
            route_constants.append(f"  static const String {screen['route_constant']} = '/{screen['route_path']}';")
        
        # Find initial screen
        initial_screen = next((s for s in screens if s["is_initial"]), screens[0] if screens else None)
        ...
        if initial_screen:
            routes_map.append(f"    initial: (context) => const {initial_screen['class_name']}(),")
        
        for screen in screens:
            routes_map.append(f"    {screen['route_constant']}: (context) => const {screen['class_name']}(),")
        
        routes_map.append("    // TODO: Add your other routes here")
        routes_map.append("  };")
        
        # Build the complete file
        file_content = self._build_complete_file(imports, route_constants, routes_map)
        
        return file_content
    
    def _build_complete_file(self, imports: List[str], constants: List[str], routes_map: List[str]) -> str:
        """
    ...
    def get_routes_analysis(self) -> Dict:
        ...
        return {
            ...
            "initial_screen": next((s for s in screens if s["is_initial"]), None)
        }


def generate_flutter_routes_for_project(lib_path: str, output_path: str = None) -> Tuple[str, Dict]:
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        try:
            content, analysis = generate_flutter_routes_for_project(lib_path, output_path)
            
            print("app_routes.dart generated successfully!")
            print(f"Screens found: {analysis['screens_found']}")
            print(f"Initial screen: {analysis['initial_screen']['name'] if analysis['initial_screen'] else 'None'}")
        ...
        print("Usage: python flutter_routes_generator.py <lib_path> [output_path]") 