...
class AIFlutterBoilerplateSetup:
    """Service to create Flutter boilerplate matching AI-generated code structure"""
    ...
    async def create_flutter_boilerplate(self) -> str:
        """Create Flutter project boilerplate for AI code generation"""
        ...
        try:
            ...
            await self._create_flutter_project()

            # Step 2: Setup project structure
            await self._create_folder_structure()

            # Step 3: Update pubspec.yaml with exact dependencies
            await self._update_pubspec_yaml()

            # Step 4: Create main.dart with exact content
            await self._create_main_dart()

            # Step 5: Create core files
            await self._create_core_files()

            # Step 6: Create widget files
            await self._create_widget_files()
            ...
            print(
                f"✅ AI Flutter boilerplate created successfully at: {self.base_path}"
            )
    ...
    async def _create_flutter_project(self):
        """Create Flutter project"""
        print("📦 Creating Flutter project...")

        cmd = ["flutter", "create", ".", "--project-name", "velocity"]
        ...
        if process.returncode != 0:
            raise Exception(
                f"Failed to create Flutter project: {stderr.decode()}"
            )
    ...
    async def _update_pubspec_yaml(self):
        """Update pubspec.yaml with exact dependencies"""
        print("📄 Updating pubspec.yaml...")

        pubspec_content = """name: velocity
...
"""
        ...
        with open(self.base_path / "pubspec.yaml", "w") as f:
            f.write(pubspec_content)

        print("✅ pubspec.yaml updated")

    async def _create_main_dart(self):
        """Create main.dart with exact content"""
...
"""
        ...
        with open(self.base_path / "lib/main.dart", "w") as f:
            f.write(main_dart_content)

        print("✅ main.dart created")

    async def _create_core_files(self):
        """Create core files"""
        print("🔧 Creating core files...")

        # app_export.dart
        app_export_content = """export 'package:connectivity_plus/connectivity_plus.dart';
...
"""
        ...
        with open(self.base_path / "lib/core/app_export.dart", "w") as f:
            f.write(app_export_content)

        print("✅ Core files created")

    async def _create_widget_files(self):
        """Create widget files"""
        print("🧩 Creating widget files...")

        # custom_error_widget.dart
        custom_error_widget_content = """import 'package:flutter/material.dart';
...
"""
        ...
        custom_image_widget_content = """import 'package:flutter/material.dart';
...
"""
        ...
        # custom_icon_widget.dart (abbreviated version with key icons)
        custom_icon_widget_content = """import 'package:flutter/material.dart';
...
"""
        ...
        # Write widget files
        with open(
            self.base_path / "lib/widgets/custom_error_widget.dart", "w"
        ) as f:
            f.write(custom_error_widget_content)

        with open(
            self.base_path / "lib/widgets/custom_image_widget.dart", "w"
        ) as f:
            f.write(custom_image_widget_content)

        with open(
            self.base_path / "lib/widgets/custom_icon_widget.dart", "w"
        ) as f:
            f.write(custom_icon_widget_content)

        print("✅ Widget files created")

    async def _setup_assets(self):
        """Setup assets directory and files"""
...
"""
        ...
        with open(self.base_path / "env.json", "w") as f:
            f.write(env_content)

        # Path to the static no_image.jpg file in backend
        static_no_image_path = Path(
            "system/backend/static/assets/images/no_image.png"
        )

        # Copy the actual no_image.jpg file from backend static assets
        if static_no_image_path.exists():
            shutil.copy2(
                static_no_image_path,
                self.base_path / "assets/images/no-image.jpg",
            )
            print("  ✅ Copied no-image.jpg from backend static assets")
        else:
            # Create placeholder image if the static file doesn't exist
            print("  ⚠️ Static no-image.jpg not found, creating placeholder")
            placeholder_content = "// Placeholder for no-image.jpg\n// This file should be replaced with actual image"
            with open(self.base_path / "assets/images/no-image.jpg", "w") as f:
                f.write(placeholder_content)

        # Create app logo SVG
        app_logo_svg = """<svg width="200" height="200" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
  <circle cx="100" cy="100" r="80" fill="#FF6B35"/>
  <circle cx="100" cy="100" r="60" fill="#FFFFFF"/>
  <text x="100" y="110" text-anchor="middle" font-family="Arial, sans-serif" font-size="24" font-weight="bold" fill="#FF6B35">V</text>
</svg>"""

        with open(self.base_path / "assets/images/img_app_logo.svg", "w") as f:
            f.write(app_logo_svg)

        # Create sad face SVG
        sad_face_svg = """<svg width="42" height="42" viewBox="0 0 42 42" xmlns="http://www.w3.org/2000/svg">
  <circle cx="21" cy="21" r="20" fill="none" stroke="#6B7280" stroke-width="2"/>
  <circle cx="15" cy="16" r="2" fill="#6B7280"/>
  <circle cx="27" cy="16" r="2" fill="#6B7280"/>
  <path d="M28 28C26.5 25.5 24 24 21 24C18 24 15.5 25.5 14 28" stroke="#6B7280" stroke-width="2" stroke-linecap="round"/>
</svg>"""

        with open(self.base_path / "assets/images/sad_face.svg", "w") as f:
            f.write(sad_face_svg)

        print("✅ Assets setup completed")

    async def _cleanup_default_files(self):
        """Remove default Flutter files that we don't need"""
        print("🧹 Cleaning up default files...")

        # Files to remove
        files_to_remove = [
            "lib/main.dart.bak",  # Backup if created
    ...
    async def _get_dependencies(self):
        ...
        if process.returncode != 0:
            print(f"⚠️ Warning: Failed to get dependencies: {stderr.decode()}")
...
setup_flutter_boilerplate = AIFlutterBoilerplateSetup()
